...
var (
	...
	IVDefaultValue = "wBAVla5EFxYiGZim"
)

func EncryptToHex(plainText []byte, key []byte, iv ...[]byte) (string, error) {
	b, err := EncryptCBC(plainText, key, iv...)
	if err != nil {
		return "", err
	}
...
func DecryptFromHex(plainTextHex string, key []byte, iv ...[]byte) ([]byte, error) {
	plainText, err := hex.DecodeString(plainTextHex)
	if err != nil {
		return nil, err
	}
	return DecryptCBC(plainText, key, iv...)
}
